console.clear();
console.log("✅ Módulo Integrantes Iniciado de forma nativa.");
...
(async () => {
  try {
    ...
    // ================================================================================================================= //
    const response = await fetch(window.API_BASE_URL +"/integrantes/list");
    const integrantes = await response.json();

    const tableBody = document.getElementById("integrantesTableBody");
    const alertInfo = document.getElementById("alert-info-integrantes");
    tableBody.innerHTML = "";
    ...
    grupos.forEach((data) => {
      ...
      // 1. Iniciamos la variable con la primera parte
        let html = `
            <td class="text-center">${data.nombre_grupo}</td>
            <td class="text-center">${data.nombre_usuario}</td>
            `;

        // 2. Concatenamos (+=) la condición correspondiente
        if (data.es_coordinador == 1) {
            html += `<td class="text-center"><span class="label label-success">${data.rol}</span></td>`;
        } else {
            html += `<td class="text-center"><span class="label label-warning">${data.rol}</span></td>`;
        }
    ...
    if ($.fn.DataTable.isDataTable("#integrantesTable")) {
      $("#integrantesTable").DataTable().destroy();
    }

    dTable = $('#integrantesTable').DataTable({
        "order": [[0, "asc"]],
        "responsive":     true,
        "scrollY":        "300px",
        "scrollX":        true,
    ...
    const contenedorModal = document.getElementById("contenedor-modal-maestro-integrantes");
    if (contenedorModal) {
        const respModal = await fetch(window.VIEWS_PATH + "/grupos/nuevo_integrante.html");
    ...
    // Evento Añadir (Alta)
    $(document).off("click", "#add-integrante-form").on("click", "#add-integrante-form", function(e) {
        e.preventDefault();
        prepararYMostrarModal("alta");
    });

    // Evento Editar (Fila de la tabla)
    $(document).off("click", ".btn-editar-integrante").on("click", ".btn-editar-integrante", function(e) {
        e.preventDefault();
        const datos = {
...
async function prepararYMostrarModal(modo, datos = null) {
    const $modal = $("#myModal-integrantes");
    if ($modal.length === 0) return;

    // Seteamos título del modal flotante
    $modal.find(".modal-title").text(modo === "alta" ? "Añadir Integrantes" : "Editar Integrantes");
